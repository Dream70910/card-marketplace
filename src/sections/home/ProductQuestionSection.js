import React from "react";

const ProductQuestionSection = () => {
  return (
    <div className="py-48">
      <div className="container mx-auto text-white product-ask-section p-20 bg-white/5 backdrop-blur-md">
        <div className="flex items-end justify-between ">
          <h2 className="uppercase font-aero text-[72px]  leading-[1.1]">
            What is{" "}
            <span className="gradient-text font-aero">
              <br />
              card shop <br />
              exchange?
            </span>
          </h2>
          <p className="w-full max-w-[370px] opacity-60 font-light">
            Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do
            eiusmod tempor incididunt ut labore et dolore magna aliqua. <br />
            <br />
            Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris
            nisi ut aliquip ex ea commodo consequat.
          </p>
        </div>

        <div className="flex justify-between pt-16 mt-16 border-t-2 border-t-white/20">
          <div className="flex items-center gap-6">
            <div className="border-style-decoration bg-primary-gradient p-6 w-fit">
              <img src="/assets/icons/icon-handshake.svg" />
            </div>
            <span className="font-aero text-xl uppercase">
              Seamless Buying <br />
              and Selling
            </span>
          </div>
          <div className="flex items-center gap-6">
            <div className="border-style-decoration bg-primary-gradient p-6 w-fit">
              <img src="/assets/icons/icon-user-switch.svg" />
            </div>
            <span className="font-aero text-xl uppercase">
              easy account <br />
              management
            </span>
          </div>
          <div className="flex items-center gap-6">
            <div className="border-style-decoration bg-primary-gradient p-6 w-fit">
              <img src="/assets/icons/icon-chat-text.svg" />
            </div>
            <span className="font-aero text-xl uppercase">
              integrated chat <br />
              functionality
            </span>
          </div>
        </div>
      </div>
    </div>
  );
};

export default ProductQuestionSection;
